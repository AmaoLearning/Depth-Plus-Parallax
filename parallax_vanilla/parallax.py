...
class Parallax:
    def __init__(self, image, depth, offset_bound=0.2, gamma=2.0):
        ...
        with torch.no_grad():
            ...
            self.depth = -depth + depth.max() # reverse depth

            self.n_depth = normalize_depth(self.depth)
    ...
    def plot_n_depth_hist(self, bins=50):
        import matplotlib.pyplot as plt
        plt.hist(self.n_depth.cpu().numpy().flatten(), bins=bins, color='blue', alpha=0.7)
        plt.title("Depth Value Distribution")
        plt.xlabel("Normalized Depth")
        plt.ylabel("Pixel Count")
        plt.show()
    ...
    def plot_depth_hist(self, bins=100):
        import matplotlib.pyplot as plt
        plt.hist(self.depth.cpu().numpy().flatten(), bins=bins, color='blue', alpha=0.7)
        plt.title("Depth Value Distribution")
        plt.xlabel("Normalized Depth")
        plt.ylabel("Pixel Count")
        plt.show()
